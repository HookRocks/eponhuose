...
const [personName,setPersonName]=useState("");
  ...
  const swalBax=() => {
    ...
    <label for="Email">Email: </label><input  onChange={(e)=>{setPersonEmail(e.target.value)}} id='Email' name='Email' type="email" placeholder='Email goes here' value={personEmail}/>
  </form></div>).then(okay => {
  //   fetch('url', {
  //   Method: 'POST',
  //   Headers: {
  ...
  if(tabbedMode) {
    ...
    return (
      <div className=' h-full text-center align-center justify-center'>
        {givenProgramName!=null? (<h1 className='font-bold text-xl'>{givenProgramName}</h1>):(<b>program list</b>)}
        <p>{givenProgramName!=null? (<h1 className='font-bold'>{programInfo[givenProgramName].instructorName.join(", ")}</h1>):(<b>All Teachers and staff</b>)}</p>
        ...
        <button onClick={(ev) => {ev.stopPropagation(); swalBax();}}><a>Program VIsited</a></button>
      </div>
    );
