...
const AdminPage=() => {
  ...
  const [programsMade,setPrograms]=useState([false,false,false,false,false,false,false,false,false,false,false,false]);
  
  const AllProgramFunc=useEffect(() => {
    var allChosen=programsMade.filter((val,ind) => val||ind==11).length==12;
      ...
      <form target='_self' method='get'>
          ...
          <fieldset className='border border-solid border-gray-300 p-3 rounded-lg'>
            <legend className='text-lg'>Programs</legend>
                <PreBuiltCheckbox boxId="select-all" boxFunction={() => { setIsAllProgChecked(!isAllProgChecked)
                  setPrograms(programsMade.map(()=>!programsMade[11]))

                }} boxContent="All Programs" className="" />
            <div className='flex align-center justify-center gap-x-9 mt-2 ml-3'>
              <div id='allClasses' className='flex align-start justify-start flex-col'>
                    <PreBuiltCheckbox boxId="Coding" boxContent="Coding" boxFunction={() => {setPrograms(programsMade.map((val,ind) => ind==0? !val:val)); console.log(programsMade)}} isCheckedAlready={programsMade[0]} />
                <PreBuiltCheckbox boxId="HVAC" boxContent="HVAC" boxFunction={() => {setPrograms(programsMade.map((val,ind)=>ind==2?!val:val)); console.log(programsMade)}} />
                <PreBuiltCheckbox boxId="ACTech" boxContent="AC Technology" boxFunction={() => {setPrograms(programsMade.map((val,ind)=>ind==1?!val:val)); console.log(programsMade)}} />
                <PreBuiltCheckbox boxId="ElectricTradeSpecial" boxContent="Electrical Trade Speciality" boxFunction={() => {setPrograms(programsMade.map((val,ind)=>ind==5?!val:val)); console.log(programsMade)}} />
                <PreBuiltCheckbox boxId="Pharmacy" boxContent="Pharmacy Science" boxFunction={() => {setPrograms(programsMade.map((val,ind)=>ind==9?!val:val)); console.log(programsMade)}} />
                <PreBuiltCheckbox boxId="VetScience" boxContent="Veterinary Science" boxFunction={() => {setPrograms(programsMade.map((val,ind)=>ind==10?!val:val)); console.log(programsMade)}} />
              </div>
              <div  className='flex align-center justify-start flex-col'>
                <PreBuiltCheckbox boxId="MA" boxContent="Medical Assisting" boxFunction={() => {setPrograms(programsMade.map((val,ind)=>ind==7?!val:val)); console.log(programsMade)}} />
                <PreBuiltCheckbox boxId="AutoTech" boxContent="Automotive Technology" boxFunction={() => {setPrograms(programsMade.map((val,ind)=>ind==3?!val:val)); console.log(programsMade)}} />
                <PreBuiltCheckbox boxId="Collision" boxContent="Collision Repair and Refinishing" boxFunction={() => {setPrograms(programsMade.map((val,ind)=>ind==4?!val:val)); console.log(programsMade)}} />
                <PreBuiltCheckbox boxId="Construction" boxContent="General Construction Technology" boxFunction={() => {setPrograms(programsMade.map((val,ind)=>ind==6?!val:val)); console.log(programsMade)}} />
                <PreBuiltCheckbox boxId="Diesel" boxContent="Diesel Technology" boxFunction={() => {setPrograms(programsMade.map((val,ind)=>ind==8?!val:val)); console.log(programsMade)}} />
                  </div>
